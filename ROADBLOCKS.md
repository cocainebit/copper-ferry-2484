...
- SDK/CLI/MCP typecheck and import but have not been exercised against a live desktop. The MCP server depends on `@modelcontextprotocol/sdk` and `zod`.
- Docker host hygiene: seven `desktop-*` volumes and the `crypto-check`/`production-check` images are leftovers from Codex smoke and build checks; they were not deleted.

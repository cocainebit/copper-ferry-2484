...
| --- | --- | --- |
| Live computer | Browser desktop, terminal, files and persistent environment. [Introduction](https://docs.orgo.ai/introduction) | Linux desktop and authenticated noVNC gateway exist. Verify public reconnect, sustained operation, keyboard/clipboard and revocation under load. |
...
| Resize/storage/network | Live CPU, bandwidth and disk growth; memory changes restart and replace connection credentials. [Resize](https://docs.orgo.ai/api-reference/computers/resize) | CPU/RAM edits require stop. No customer disk tier, hard disk quota or bandwidth setting. Implement truthful resize jobs, effective configuration, disk growth, bandwidth limits and reconnect handling. |
| Always-on/idle policy | Idle timeout is configurable; zero disables it. [Resize](https://docs.orgo.ai/api-reference/computers/resize) | Worker stops inactive desktops after 15 minutes. Add persisted per-computer policy; account for paid background workloads independently of browser activity. |
| Resolution/multiple screens | Screen lifecycle supports up to four displays and screen-targeted input. [Screens](https://docs.orgo.ai/api-reference/screens/create) | Single fixed 1440×900 screen at audit time. Add resolution choices, per-screen view/control and screen-specific agent coordinates. |
...
| Public computer API | Lifecycle plus screenshot/click/drag/type/key/scroll/wait/bash/exec operations. [API index](https://docs.orgo.ai/llms.txt) | Session-authenticated lifecycle, one-shot terminal and internal agent tools exist. Expose scoped computer primitives to external agents with rate limits, consistent errors, cancellation and isolation tests. |
| API credentials | User-generated account/workspace API keys, scope and rotation. [Authentication](https://docs.orgo.ai/api-reference/authentication) | Supabase sessions and service billing credentials exist; neither is a full customer computer API-key system. Add hashed keys, one-time reveal, expiry/revoke, least-privilege computer/workspace scopes and audit history. |
| Developer clients | REST, Python/TypeScript SDKs, CLI and MCP. [Introduction](https://docs.orgo.ai/introduction), [CLI](https://docs.orgo.ai/guides/cli), [MCP](https://docs.orgo.ai/guides/mcp) | Shared billing client exists, not a desktop SDK. Deliver computer SDKs, CLI and MCP backed by the same contract and examples; test a third-party agent without the dashboard. |
| Interactive terminal | Persistent interactive PTY connection, including CLI shell access. [Terminal](https://docs.orgo.ai/api-reference/computers/terminal) | HTTP command runs with a short timeout; not an interactive PTY. Add resize/input/output, disconnect/reconnect semantics, escape handling and authorization revocation. |
...
| Files | Upload, export, list, download and delete APIs. [File API index](https://docs.orgo.ai/llms.txt) | Listing and home download exist; download capped at 20 MB. Add authenticated binary upload/export/delete, quotas, safe paths and progress; verify symlink escapes and partial transfers. |
